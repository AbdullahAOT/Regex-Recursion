...
function capitalA(s){
    let str=s.replace(/ /g,"");
    let regexCheckForNonLetter=/\W/g;
    if(regexCheckForNonLetter.test(str)){
        return false;
    }
    let regexCheckForA_LetterInTheEnd=/A$/;
    if(!regexCheckForA_LetterInTheEnd.test(str)){
        return false;
    }
    return true;
}
...
function ioEmail(email){
    let regexCheckForEmail = /^[a-zA-Z]+@[a-zA-Z]+.io$/;
    if(regexCheckForEmail.test(email)){
        return true;
    }
    return false;
}
...
function imagesSearcher(text){
    let regexCheckForImages = /[a-zA-Z]+\.(jpg|jpeg|png)\b/g;
    let arrayOfImagesFound = text.match(regexCheckForImages);
    return arrayOfImagesFound || [];
}
